...
- **⚓ 语义锚点定位**：结合 A11y 辅助功能树、视觉模糊特征与多级兜底策略，即使网页 UI 变动，引擎也能自愈定位。
- **🔍 视觉证据链**：全链路透明监控。系统自动记录网络请求、DOM 变更及关键节点的 "黑匣子截图"。
- **🛡️ 故障自愈与降级**：内置线性回退重试算法。当网页端失效时，自动降级至本地 Ollama 方案。
- **📈 全异步任务系统**：基于 `aiosqlite` 实现的非阻塞任务追踪与存储，支持高性能并发处理。
- **⚡ 引擎优化与安全**：引入了 UI 懒加载机制与增强型 `BrowserPool`。通过单一全局 Playwright 实例管理与严格的 atexit 资源回收，彻底杜绝僵尸进程，并集成了 CI 级的 AI 平台 UI 兼容性探针任务。
