...
async def check_environment():
    ...
    # 检查依赖
    checks.append(("PyJWT", importlib.util.find_spec("jwt") is not None, "installed" if importlib.util.find_spec("jwt") else "missing"))
    checks.append(("cryptography", importlib.util.find_spec("cryptography") is not None, "installed" if importlib.util.find_spec("cryptography") else "missing"))
    checks.append(("Playwright", importlib.util.find_spec("playwright") is not None, "installed" if importlib.util.find_spec("playwright") else "missing"))
    
    # 检查环境变量
    jwt_secret = os.getenv('SHADOW_JWT_SECRET')
    checks.append(("SHADOW_JWT_SECRET", bool(jwt_secret), 
                  "configured" if jwt_secret else "missing"))
    
    master_key = os.getenv('SHADOW_MASTER_KEY')
    checks.append(("SHADOW_MASTER_KEY", bool(master_key),
...
async def init_auth_system():
    ...
    try:
        ...
        admin = await auth.create_user(
            ...
            password="admin@shadowboard!123",  # 生产环境应修改
            role=Role.ADMIN,
        )
        ...
        operator = await auth.create_user(
            ...
            password="operator@shadowboard!123",
            role=Role.OPERATOR,
        )
        ...
        viewer = await auth.create_user(
            ...
            password="viewer@shadowboard!123",
            role=Role.VIEWER,
        )
        ...
        print("\n✨ 认证系统初始化完成！")
        print("\n📋 初始凭证 (生产环境请修改):")
        print("  Admin: admin / admin@shadowboard!123")
        print("  Operator: operator / operator@shadowboard!123")
        print("  Viewer: viewer / viewer@shadowboard!123")
